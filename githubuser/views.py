...
class GitHubProjects(APIView):
                ...
                projects.append(
                    {
                        ...
                        "id": project["id"],
                        "added": self.check_if_project_is_added(project["name"], project["id"], request.user),
                        "loading": False,
                    }
                )
            next_page_link = self.get_next_page_link(response.headers)
            next_page_number = self.get_page_number(next_page_link) if next_page_link else None
            projects_response = {
                "projects": projects,
                "pagination": {
    ...
    def check_if_project_is_added(self, repo_name, project_id, user):
        try:
            repo = PrivateRepository.objects.get(repo_id=project_id, user=user, repo_name=repo_name)
            return True
...
class GitHubAddProject(APIView):
    def post(self, request):
        ...
        try:
            ...
            pprint(data)
            # check if exists 
            if PrivateRepository.objects.filter(repo_id=data.get("id"), user=user).exists():
            ...
            repo.save()
            return JsonResponse({"message": "Project added successfully"}, safe=True, status=200)
        except Exception as e:
            print("Error al agregar el proyecto:", str(e))
            return JsonResponse({"error": "Error al agregar el proyecto"}, safe=True, status=500)
...
class UserTokens(APIView):
    ...
    def get(self, request):
        user = request.user
        try:
            tokens = UserTokensModel.objects.filter(user=user).annotate(
            ...
            for token in tokens:
                access_tokens.append(
                    {
                        "value": self.hide_last_chars(token["token"]), "created_at": token["created_date"], "type": "personal"
                    }
                )
            response = {
                "data": access_tokens
            ...
            return JsonResponse({"error": "Error al obtener los tokens del usuario"}, safe=True, status=500)
    ...
    def post(self, request):
        ...
        try:
            ...
            if UserTokensModel.objects.filter(user=user, access_token=token).exists():
                return JsonResponse({"error": "Token already exists"}, safe=True, status=400)
            user_token = UserTokensModel(user=user, access_token=token)
            user_token.save()
            return JsonResponse({"message": "Token added successfully"}, safe=True, status=200)
        except Exception as e:
            print("Error al obtener los datos del usuario:", str(e))
            return JsonResponse({"error": "Error al obtener los datos del usuario"}, safe=True, status=500)